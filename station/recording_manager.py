...
class RecordingManager:
    def __init__(
        ...
        *,
        ...
        self._chunk_index = 0
        self._chunk_audio: list[np.ndarray] = []
    ...
    def stop_recording(self) -> dict[str, Any]:
        with self._lock:
            if self._recording:
                self._flush_chunk_locked(time.time())
    ...
    def append_audio(self, audio: np.ndarray, timestamp: float | None = None) -> None:
        with self._lock:
            ...
            self._chunk_audio.append(array.copy())
            self._chunk_samples += int(array.shape[0])
            while self._chunk_samples >= self._chunk_sample_limit():
                self._flush_chunk_locked(now, target_samples=self._chunk_sample_limit())
    ...
    def _flush_chunk_locked(self, now: float, target_samples: int | None = None) -> None:
        ...
        merged = np.concatenate(self._chunk_audio, axis=0)
        write_audio = merged[:target]
        remaining = merged[target:]
        self._chunk_audio = [] if remaining.size == 0 else [remaining]
        self._chunk_samples = 0 if remaining.size == 0 else int(remaining.shape[0])
        chunk_start = now - float(write_audio.shape[0]) / float(self.sample_rate)
