...
SkyEar field sessions are engineering dry-runs only, not operational deployments. The goal is to collect clean acoustic, station-health, and ground-truth notes for later evaluation and model training.
...
- Do not use SkyEar output for targeting, weapon integration, or public warning decisions.
